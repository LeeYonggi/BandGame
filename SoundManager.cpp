...
void SoundManager::RePlay(string str, bool loop)
{
	auto iter = m_Sound.find(str);
	if (iter == m_Sound.end()) return;
	...
	if (loop == true)
	{
		mciSendCommand(iter->second, MCI_RESUME, 0, MCI_DGV_PLAY_REPEAT);
	}
	else
	{
		mciSendCommand(iter->second, MCI_RESUME, 0, MCI_NOTIFY);
	}
}
...
void SoundManager::Close(string str)
{
	...
	m_Sound.erase(iter->first);
}

void SoundManager::AllPause()
{
